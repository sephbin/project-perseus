...
class ElementDeltaSubmissionView(ReadOnlyModelViewSet):
	...
	def create(self, request, *args, **kwargs):
		...
		try:
			sourceModel = data["documentGuid"]
			sourceModel, _created = Source.objects.get_or_create(unique_id=sourceModel)
			eventOb = Event(event_type="Revit End Sync", source_model=sourceModel, user=user)
			print(eventOb)
			eventOb.save()
		except Exception as e: print(e)

		print(data)
		try:	print(data["elements"][0])
		except: pass
		...
		# data = data[4090:]
		# print(data[0])
		serializer = self.create_serializer_class(data=data["elements"])
